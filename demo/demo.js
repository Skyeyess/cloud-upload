...
function putObject(currentName, currentPath) {
    // 创建测试文件
    var filename = currentName || '';
    var filepath = currentPath || path.resolve(__dirname, filename);
        ...
        cos.putObject({
            ...
            onProgress: function (progressData) {
                if (progressData.percent) {
                    console.log(`[upload-progress]: ${filename.split('/').pop()} upload progress ${progressData.percent * 100}%.`);
                } else {
            ...
            if (data && data.Location) {
                console.log(`[upload-success]: ${data.Location.split('/').pop()} upload to cloud success.`);
                fs.unlinkSync(filepath);
            } else {
                if (err.error && err.error.code) {
                    console.log(`[upload-fail]: ${err.error.code}.`);
                } else {
                    console.log(err || data);
                }
            }
...
function uploadFiles() {
    ...
    if (uploadArr.length) {
        ...
        console.log('[upload-empty]: upload directory no files.');
    }
}
